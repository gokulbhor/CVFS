...
//Macro che name capital
#define REGULAR 1
#define SPECIAL 2//make directory
//offset change karnyasathi
#define START 0
#define CURRENT 1
#define END 2
...
{
    char FileName[50];//(max file size)
    int InodeNumber;
    int FileSize;
    int FileActualSize;
    int FileType;
    char *Buffer;//pointer to points actual data
    int LinkCount;//always 1
    int ReferenceCount;
    int permission; // 1 2 3(read,write,read+write)
    struct inode *next;
...
typedef struct ufdt//structure cha ek array create kelaay jo 
{
...
void CreateDILB()
{
    ...
    while(i <= MAXINODE)
    {
        ...
        else//insert next//create new node
        {
            temp->next = newn;
            temp = temp->next;
        }
...
int CreateFile(char *name, int permission)
{
    ...
    while(temp != NULL)
    {
        if(temp->FileType == 0)//inode kuni use karat nahi hai
            break;
        temp = temp->next;
    }
...
int main()
{
    ...
    while(1)
    {
        ...
        count = sscanf(str,"%s %s %s %s",command[0],command[1],command[2],command[3]);//tokens gets formed in scanf//we used 4/80//india is my country//sscanf tukde karto
		//sscan split in delimeter
        if(count == 1)
        {
            ...
            {
                system("cls");//program through cmdlaa command dete
                continue;
            }
     ...
     else if(count == 4)
     {
         if(strcmp(command[0],"lseek") == 0)
         {
             ...
             ret = LseekFile(fd,atoi(command[2]),atoi(command[3]));//string to int
             if(ret == -1)
             {
